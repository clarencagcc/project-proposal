...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("region", "Select Region:", choices = unique(df$who_region)),
      selectInput("start_year", "Start Year:", choices = unique(df$measurement_year)),
      selectInput("end_year", "End Year:", choices = unique(df$measurement_year))
    ),
    mainPanel(
      DTOutput("table"),
      plotlyOutput("pm25_plot"),
